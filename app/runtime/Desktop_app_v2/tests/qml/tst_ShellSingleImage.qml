...
Item {
    ...
    QtObject {
        ...
        property var modelRows: [
            {
                ...
                classCount: 2, classSummary: "Empty, Single", active: true
            },
            {
                id: "model-candidate", name: "Candidate Model",
                architecture: "mobilenet_v3_small", performanceLabel: "Faster",
                classCount: 3, classSummary: "Empty, Single, MoreThanOne", active: false
            }
        ]
        ...
        readonly property var selectedDetail: selectedIndex >= 0 ? {
            ...
            classCount: modelRows[selectedIndex].classCount,
    ...
    function test_liveDisclosureRules() {
        ...
        shell.form.liveWorkspace.triggerTimingHeadingButton.clicked()
        verify(!shell.mockState.liveTriggerTimingExpanded)
        verify(!shell.form.liveWorkspace.triggerTimingExpanded)
        shell.form.liveWorkspace.triggerTimingHeadingButton.clicked()
        verify(shell.mockState.liveTriggerTimingExpanded)
        verify(shell.form.liveWorkspace.triggerTimingExpanded)
        ...
        verify(!shell.form.liveWorkspace.triggerTimingExpanded)
        verify(!shell.form.liveWorkspace.triggerTimingHeadingButton.enabled)
        shell.mockState.toggleLiveTriggerTiming()
        verify(shell.mockState.liveTriggerTimingExpanded)
        verify(shell.form.liveWorkspace.runningExpanded)
        verify(shell.form.liveWorkspace.runningHeadingEnabled)
        shell.form.liveWorkspace.runningHeadingButton.clicked()
    ...
    function test_trainingControllerDirectWiring() {
        ...
        tryVerify(function() {
            return selector.popup.visible
                    && selector.popup.contentItem.itemAtIndex(0) !== null
    ...
    function test_liveAndSequenceTestUseProductionControllers() {
        ...
        verify(shell.form.liveWorkspace.runningExpanded)
        verify(shell.form.liveWorkspace.runningHeadingEnabled)
        verify(shell.form.liveWorkspace.hitClassControl.enabled)
        verify(shell.form.liveWorkspace.triggerEveryDropletControl.enabled)
        verify(shell.form.liveWorkspace.daqOutputControl.enabled)
        shell.form.liveWorkspace.hitClassControl.activated(0)
        compare(liveSortingController.hitClassId, "2")
        shell.form.liveWorkspace.triggerEveryDropletControl.click()
        compare(liveSortingController.triggerEveryDroplet, true)
        shell.form.liveWorkspace.daqOutputControl.click()
        compare(liveSortingController.daqOutputEnabled, false)
        verify(!shell.form.liveWorkspace.sendTestPulseButton.enabled)
        shell.form.liveWorkspace.triggerTimingHeadingButton.clicked()
        verify(!shell.form.liveWorkspace.triggerTimingExpanded)
        shell.form.liveWorkspace.triggerTimingHeadingButton.clicked()
