...
- Sequence Test tab
  - Internal GUI-only test controls may be present, but standalone sequence replay validation tooling is excluded from this internal release package.
...
- Per-run folders
  - In this repo layout, generated results should go under `../../outputs/pipeline_output/`
    when running from `app/runtime/`.
  - Each live run creates: `../../outputs/pipeline_output/live_YYYYMMDD_hhmmss`
...
Build (PowerShell)
Keep the build directory outside this clean release repo. From `app/runtime/`, configure and build with CMake:

```powershell
cmake -S . -B "..\..\..\build-internal-release" `
  -G "Visual Studio 17 2022" -A x64 `
  -D CMAKE_PREFIX_PATH="C:\Qt\6.10.1\msvc2022_64" `
  -D ONNXRUNTIME_DIR="C:\onnxruntime-gpu" `
  -D DCAM_SDK_DIR="C:\path\to\dcamsdk4" `
  -D NIDAQMX_DIR="C:\Program Files (x86)\National Instruments\NI-DAQ\DAQmx ANSI C Dev" `
  -D ENABLE_NIDAQMX=OFF `
  -D BUILD_QT_GUI=ON
cmake --build "..\..\..\build-internal-release" --config Release --target desktop_app
```

Run
- GUI:
  `..\..\..\build-internal-release\desktop_app\Release\OpenVisualDropletSorter.exe`
- CLI (example, uses same exe):
  `..\..\..\build-internal-release\desktop_app\Release\OpenVisualDropletSorter.exe` --cli ^
    ...
    --output-dir ../../outputs/pipeline_output
Notes
...
- The GUI is the primary entry point for live operation.
