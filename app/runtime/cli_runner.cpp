...
int run_cli(int argc, char** argv) {
    ...
    if (metaPath.empty()) {
        ...
        candidates.push_back(fs::current_path() / "app" / "runtime" / "models" / defaultMetaName);
        appendUpwardCandidates(candidates, exeDir, fs::path("models") / defaultMetaName);
        appendUpwardCandidates(candidates, exeDir, fs::path("app") / "runtime" / "models" / defaultMetaName);
    ...
    std::cout << "Target class: " << FormatClassForDisplay(targetClassId, targetDisplayLabel) << "\n";

    DcamCamera camera;
    int deviceIndex = getInt(args, "--device", 0);
    err = camera.init(deviceIndex);
    if (!err.empty()) {
    ...
    if (!classifier.init(onnxPath, meta, false, err)) {
