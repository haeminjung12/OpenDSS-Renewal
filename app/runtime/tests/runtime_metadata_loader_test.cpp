...
int main() {
    ...
    if (!require(metadata.classes.size() == 2, "metadata should contain two classes")) return 1;
    if (!require(metadata.displayLabels.size() == 2, "metadata should contain two display labels")) return 1;
    if (!require(metadata.displayLabels[1] == "Hit", "class 1 should resolve to display label Hit")) return 1;
    if (!require(metadata.inputH == 64 && metadata.inputW == 64 && metadata.inputC == 1,
                 "input_size should populate H/W/C")) return 1;
    if (!require(metadata.mean.size() == 1 && metadata.mean[0] == 0.5f,
                 "mean vector should load the configured normalization mean")) return 1;
    if (!require(metadata.std.size() == 1 && metadata.std[0] == 0.25f,
                 "std vector should load the configured normalization std")) return 1;
    if (!require(DisplayLabelForClassId(metadata, "1") == "Hit",
                 "DisplayLabelForClassId should return the mapped display label")) return 1;
    if (!require(FormatClassForDisplay("1", "Hit") == "Hit (1)",
                 "FormatClassForDisplay should include both label and class id")) return 1;
    ...
    if (!require(resolvedClassId == "1" && resolvedDisplayLabel == "Hit",
                 "display label resolution should choose class 1")) return 1;
    ...
    if (!require(resolvedClassId == "1" && resolvedDisplayLabel == "Hit",
                 "default binary target should resolve to class 1 / Hit")) return 1;
    ...
    if (!require(err.find("not present") != std::string::npos,
                 "missing class id error should explain the lookup failure")) return 1;
