...
- The slice remains open only for user-led overall visual validation in Qt Design Studio 2D view and Live Preview at 1600 × 900, maximized, and a larger 16:9 size, including focus and non-color cues. No GUI automation was run for the refreshed audit/validation; the earlier attempt was interrupted by user Escape and its first launch lacked the MinGW runtime on `PATH`, so it is not accepted evidence.
