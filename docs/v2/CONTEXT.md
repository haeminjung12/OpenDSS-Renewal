...
- `UAT-CONFIG-001` is user-approved authority for the next bounded configuration slice: all visible `Hardware Configuration` panel/action wording becomes `Configuration`; Configuration adds the sole approved `Small-droplet rejection` detector control, whose source-pixel rectangle area sets the existing minimum contour-area threshold. The current value is saved in Setup Profile, remains editable and applies immediately during active Runs, and is never written to Run/event/log/Results artifacts. Protected detector mechanics remain unchanged except through the smallest qualified authoritative-state boundary.
- `UAT-ROUTE-002` supersedes every earlier routing-geometry interpretation: the operational `Decision Boundary` requires explicit Set, one source-image X/Y click, and a horizontal segment from that point to the RIGHT EDGE only. Ordinary clicks are inert; Top/Bottom selects Hit side; Reset clears; Start is blocked while unset. Live and Sequence Test own independent workspace-local state, and coordinates never enter profiles, Runs, events, logs, Results, exports, or history. All in-frame positions, including left-of-X, are tracking-only. Classification occurs only when the track ends/disappears, using final Y against boundary Y. Clicked X positions only the visible segment. Even a track ending before X receives final-Y Hit/Waste classification; no Unresolved outcome is introduced.
- `UAT-VIEWER-001` defines presentation-only navigation for shared full-size image viewers: cursor-centered `Ctrl`+wheel zoom, fit-relative `1.0`, clamp `0.3`–`10`, and ordinary scroll/pan. Grids and thumbnails are excluded; source coordinates, artifacts, detector values, and operational geometry are unchanged.
- `UAT-TRAIN-001` replaces offline/embedded training payload requirements. A signed bootstrap installer may require internet and downloads exact pinned CPython `3.12.10` plus the authoritative 37-wheel hash-locked training environment, verifies every hash, provisions `%LOCALAPPDATA%\OpenDSS\training-venv-gpu`, runs the authoritative environment check, and fails atomically/visibly. After successful installation, Training remains local with no runtime network fallback; trained-model local/no-network requirements are unchanged.
- `SCHED-001` keeps every active worker and approved CONFIG/ROUTE/VIEWER/LABEL lane. The immediate delivery gate is a functional current-machine Training workspace using exactly `%LOCALAPPDATA%\OpenDSS\training-venv-gpu`; only fresh-machine installer qualification is deferred. `UAT-MODEL-005` Dataset Validation batching is the highest-priority backend feature and proceeds in parallel whenever an independent capacity slot is available.
- `UAT-MODEL-005` authorizes automatic high-throughput Python/PyTorch batching for the existing Model Test/Dataset Validation operation without changing its visible workflow, scientific semantics, artifacts, metrics, hashes, or recovery contract. Training and Dataset Validation use the local Python/PyTorch backend at `%LOCALAPPDATA%\OpenDSS\training-venv-gpu`, automatic GPU-first with CPU fallback; they do not use C++ ONNX Runtime. Live and Sequence real-time sorting retain ONNX Runtime and default CPU. The runtime selects the largest qualified memory-safe batch automatically; no user batch setting exists. Per-image facts/order are preserved, completed batches are atomic persistence checkpoints, Stop begins no later batch, and allocation failure reduces batch size before using existing truthful failure behavior.
- `UAT-CONFIG-BLK-002` is limited to the initial Small-droplet rejection state: protected `FastEventConfig::minArea == -1.0` means automatic, while the current master expects a concrete displayed/saved source-pixel area. User authority must choose either an exact initial pixel-area value or an `Automatic` presentation plus its concrete-resolution rule. No agent may infer a value or add a second authoritative detector state; other lanes continue.
- `UX-WINDOW-002` supersedes only the no-restore/no-fixed-minimum portion of `UX-WINDOW-001`. OpenDSS still starts maximized. Restore Down is supported; the restored window enforces an exact 1600 × 900 logical-px minimum, may grow larger, and never resizes below it. Validation may use maximized state and restored state at exactly 1600 × 900 or larger, never below.
...
- The user granted standing hardware authorization. Validation owns announced Camera/DAQ HIL on the accepted artifact; Integration remains the sole writer and corrects exact returned defects.
