...
- `UAT-CONFIG-001` is user-approved authority for the next bounded configuration slice: all visible `Hardware Configuration` panel/action wording becomes `Configuration`; Configuration adds the sole approved `Small-droplet rejection` detector control, whose source-pixel rectangle area sets the existing minimum contour-area threshold. Its initial authoritative value is exactly `100 px²`; the product does not expose or retain `Automatic` or `-1` as the default. The current numeric value is saved in Setup Profile, remains editable and applies immediately during active Runs, and is never written to Run/event/log/Results artifacts. Protected detector mechanics remain unchanged except through the smallest qualified authoritative-state boundary.
