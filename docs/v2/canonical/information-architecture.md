...
| **Data > Label** | Dominant Droplet Crop grid; right-side Selected Crop and Classes & Filter collapsible sections. |
...
| **Models > Train** | Pre-start Dataset, Faster/More Accurate, Model Name, Save Location; live progress/timing/device and two minimal plots; automatic save and activation; results tables and optional confusion matrix. |
| **Models > Model Test** | Uses the Active Model; no local Model selector. |
...
| **Ready** | Dataset/classes/counts, Model Type, Model Name, Save Location, and factual automatic device selection. | **Start Training** enabled only when all pre-start inputs are valid. | — | Start Training. |
...
| **Running** | Elapsed time, epoch/overall progress, estimated remaining time, automatic device, Training/Validation Loss plot, and Validation Accuracy plot. | **Stop Training** enabled. | Another Start is disabled because **Training is active**. | Continue or stop. |
...
| **Completed** | Overall-results and per-class tables, optional confusion matrix, saved path, and Active Model confirmation. | **Open in Model Test** enabled. | — | Test the Active Model or open Library. |
