...
| **Models > Train** | Pre-start Dataset, Architecture, Weights, Compute Device, Model Name, and Save Location; GPU requested by default with CPU selectable; requested/effective device status; MobileNet/Faster and EfficientNet/More Accurate presentation; Dataset Summary in a main white region; separate main white Results region below for live progress, the two approved plots, and completion tables; automatic save and activation. |
...
| **Settings** | Centered Storage, Application Information, Diagnostics, and Visuals only; Visuals contains one Text Size dropdown with exactly 80%, 100%, 125%, 150%, 175%, and 200%, default 100% at an approximately 22 px body baseline. |
...
The exact outer-panel titles are **Capture**, **Label**, **Train**, **Model Test**, **Library**, **Live**, **Sequence Test**, and **Runs**. Each uses a light top strip with its title left-aligned while expanded and the existing narrow chevron toggle fixed on the panel's right edge at vertical center, with the same screen x/y position in expanded and collapsed states. When collapsed width is insufficient, only the fixed chevron may remain visible. The directly draggable default width is approximately **536 px at 100% Text Size**, not fixed, and scales with Text Size. Settings has no outer collapsible panel.
...
| **Models > Train** | Trains a MobileNet or EfficientNet two-class or three-class model from Labeled Droplet Crops, using ImageNet-pretrained or a compatible OpenDSS droplet-trained checkpoint. |
...
| **Settings** | Provides Storage, Application Information, Diagnostics, and Visuals without duplicating Camera or DAQ controls; Visuals contains only the exact Text Size choices 80%, 100%, 125%, 150%, 175%, and 200%, default 100% at an approximately 22 px body baseline. |
...
| **Models > Train** | Train a two-class or three-class droplet-classification model. | Dataset Summary in a main white region; Architecture (MobileNet with Faster supporting text or EfficientNet with More Accurate supporting text); Weights (ImageNet-pretrained or a compatible bundled/user-added OpenDSS droplet-trained checkpoint); Compute Device with GPU selected by default and CPU selectable; requested/effective device status; Model Name; Save Location; separate main white Results region below for progress/timing, Training/Validation Loss plot, Validation Accuracy plot, and completion tables. | **Start Training** after all pre-start fields are valid. | Stop Training; Retry Save after final-save failure; open the saved Active Model in Model Test or Library. | Compatible labeled Dataset; compatible weights; bundled training environment; writable temporary and final locations; global long-running-operation slot. A GPU request may fall back to effective CPU with a direct explanation; a CPU request stays CPU. | Automatically saved Model Package containing `metadata.json`, `checkpoint.pth`, and `model.onnx`, including requested and effective device; successful save makes it Active. | **Open in Model Test** or review in **Models > Library**. |
...
| **Settings** | Manage storage and the sole visual preference while inspecting local application/runtime information. | Storage; Application Information; Diagnostics; Visuals; default data root; OpenDSS and schema versions; runtime and driver availability; diagnostic-folder access; one Text Size dropdown with exactly 80%, 100%, 125%, 150%, 175%, and 200%, default 100% at an approximately 22 px body baseline. | **Choose Default Data Root** | Open data root in Windows Explorer; open diagnostic folder; set Text Size. | No hardware. A selected data root must be a valid writable location before it becomes the default. | Updated SettingsRepository storage or Text Size preference state; no scientific artifact. | Return directly to any workspace. |
...
| State | What the user sees | Primary action | Direct disabled reason | Recovery or next action |
|---|---|---|---|---|
| **Ready** | Storage, Application Information, Diagnostics, Visuals, version information, runtime/driver availability, current data root, and one Text Size dropdown with exactly 80%, 100%, 125%, 150%, 175%, and 200%, default 100% at an approximately 22 px body baseline. No other setting is present. | **Choose Default Data Root** enabled. | — | Select a writable root, open the current data/diagnostic folder, or set application-wide Text Size. |
...
| **WF §15 — Train a Model** | Models > Train | Preserves Dataset selection, approved Architecture and compatible Weights selection, fixed 70/15/15 split, seed 1729, metrics, Stop, model naming/saving, and automatic activation. Compute Device is a normal selection with GPU requested by default and CPU selectable; requested/effective device are recorded, requested GPU falls back to effective CPU when unavailable, and requested CPU is never promoted. MobileNet carries the Faster supporting label and EfficientNet carries More Accurate. No technical hyperparameters are exposed. |
...
| **WF §23 — Settings** | Docked Hardware panel plus reduced Settings | Camera and DAQ technical settings remain in the shell panel. Settings retains only Storage, Application Information, Diagnostics, and Visuals; Visuals contains exactly 80%, 100%, 125%, 150%, 175%, and 200% Text Size choices, default 100% at an approximately 22 px body baseline. Detection, crop, sorting-algorithm, internal timing, and training-hyperparameter controls are not exposed. |
...
| **D-015 — No Advanced Training Parameters** | Train exposes Dataset, Architecture, compatible Weights, a GPU-default/CPU-selectable Compute Device with requested/effective status, progress, metrics, Stop, and model save. Technical hyperparameters remain qualified configuration and are not exposed. |
...
| **D-017 — Reduced Settings** | Storage, Application Information, Diagnostics, and Visuals only; Visuals contains exactly 80%, 100%, 125%, 150%, 175%, and 200% Text Size choices, default 100% at an approximately 22 px body baseline. |
...
| **Advanced Training Parameters** | Removed. The user selects Architecture (MobileNet/Faster or EfficientNet/More Accurate), compatible Weights, and Compute Device. Compute Device is a normal execution selection, not a hyperparameter. The effective qualified hyperparameters, 70/15/15 split, and seed 1729 remain fixed and recorded. |
