...
- OpenDSS launches maximized and retains Minimize, Maximize/Restore, and Close. No fixed logical minimum or restored-window resolution is a product or validation requirement. Content fills the maximized window's full available work area through the bottom. GUI, Qt Design Studio, and Computer Use validation use the maximized window only and do not restore or resize it to a fixed test resolution.
