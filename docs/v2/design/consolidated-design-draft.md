...
Final Verification uses aligned result rows and displays the result and exact failure for each installed item it verifies: OpenDSS, both bundled three-class Model Packages, Library registration, and the single Active MobileNetV3-Small package. It may repeat only the same optional DCAM-runtime-software and NI-DAQmx-runtime-software presence facts from Prerequisite Check; it never probes or reports connected hardware, DCAM driver/device presence, Camera/DAQ operation, NVIDIA/GPU/CUDA, Python, Training Environment health, training packages, or effective Training compute. It never reports an unavailable package, registration, or Active Model as ready. **Run training setup now** may appear as the optional handoff to the separate post-install verifier; it is not an installer verification result. That action and final navigation actions appear in one consistent bottom action area; no Repair action floats beside or between verification rows.
