...
| `MODEL-WHEEL-SUCCESS-LOG-001` | FUNCTIONAL diagnostics owner; VALIDATION static acceptance owner | Exact bounded correction authorized; no execution | Read-only diagnosis isolates the failure to the existing-destination `System.IO.File.Replace(temp, log, null, true)` call in `Write-UpdaterDiagnostic` while writing final `status=succeeded`. Disk, filesystem, ownership, attributes, ACL inheritance, capacity, and temporary residue show no persistent fault. The exact exception/HResult is unrecoverable because the accepted best-effort catch emitted only a fixed warning; do not infer sharing, antivirus, or another cause. Change only that post-commit catch to emit a bounded exception type and message non-terminating to stderr so a future durable command log retains the cause, while preserving updater success and exit 0. Do not change the replacement strategy, touch the installed environment, or rerun the updater. Require changed-lines static review only. |
| `VAL-MODEL-005-REP` | FUNCTIONAL probe correction owner; VALIDATION static/execution acceptance owner | Exactly-once run FAILED SAFE before backend; exact probe-only correction authorized; no execution | Controlling master: `ODSS-DES-002` §§13.3, 13.5–13.7. The registered frozen `c4f1e47` target stopped after 0.65 s because its six-frame fixture set `sourceFrameIndex` from the original zero-based/later audit `manifestIndex`; `DatasetManifestV2` requires fixture-local values 1 through 6, causing the deterministic first Neutral rejection. Python/backend/pip did not execute; TEMP cleanup passed with zero residue; the accepted installed environment remains unchanged. In the existing probe only, set `sourceFrameIndex` to fixture index + 1 and preserve the original `manifestIndex` in `sourceFrameId`/order evidence. No production or CMake changes. Require changed-lines static review; do not execute or retry until separately released. Do not rerun updater/CLI gates, expand to the full Dataset, launch GUI, or permit network access. |
