...
| `VAL-MODEL-005-REP` | FUNCTIONAL probe correction owner; VALIDATION static/execution acceptance owner | Exact correction integrated `9965f44`; changed-lines static acceptance pending; no execution | Controlling master: `ODSS-DES-002` §§13.3, 13.5–13.7. The prior frozen target failed safely before backend because its six-frame fixture reused the original audit `manifestIndex` as `sourceFrameIndex`, outside required fixture-local 1 through 6. Integrated `9965f44` changes exactly one line to fixture index + 1 while preserving original `manifestIndex` in `sourceFrameId`/order evidence. Release target build passes and diff is clean; no execution occurred. Require Validation changed-lines static acceptance and a separate later execution release. Do not alter production/CMake, rerun updater/CLI gates, expand to the full Dataset, launch GUI, or permit network access. |
