...
| `UAT-CONFIG-001` | FUNCTIONAL artifact/correction owner; DESIGN protected-form owner; VALIDATION acceptance owner | Integrated `17bb4b0`; static HOLD pending sanitized-`PATH` correction/review | Product behavior remains unchanged: visible `Configuration`; sole `Small-droplet rejection`; nearest-pixel rectangle mapping; exact `100 px²` initial/legacy value; profile-only current numeric persistence; immediate active-Run effect; zero Run/event/log/Results provenance. Detector non-GUI and existing Shell offscreen registrations are acceptable. `live_sorting_controller_test` must receive the smallest central-harness sanitized-`PATH` correction and scoped review acceptance before any Configuration execution. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled parallel execution | Retain every active worker. Continue CONFIG, ROUTE, LABEL, Training, and Model Test integration in parallel with `UAT-TRAIN-INSTALL-001` setup/preflight/dry-run work. Training backend and Model Test C++ gates are closed; do not repeat their evidence. Continue `MODEL-WHEEL-LOCK-001` and Configuration sanitized-`PATH` correction; both affected execution lanes remain held. All unregistered executable paths remain blocked. VIEWER is accepted. |
