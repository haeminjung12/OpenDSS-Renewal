...
| `VAL-MODEL-005` | FUNCTIONAL Model Test correction owner; VALIDATION scoped-rereview/acceptance owner | Partial PASS; service/Python execution held | Summary passed in 0.46 s and writer in 11.05 s. Service waits for scoped rereview of the forced-stop guard correction. Python unit and real-Dataset execution remain held by `MODEL-WHEEL-LOCK-001`. Do not broaden to product, QML, or GUI execution. |
| `MODEL-WHEEL-LOCK-001` | FUNCTIONAL deterministic-build/authority-update owner; VALIDATION scoped-review owner | Authority reconciliation blocker; no user decision | Installed exact OpenDSS Python correctly lacks the new backend/test. Source-built wheel SHA `b28f56b9…b85943` differs from authoritative lock `fd00e2a3…66a747`; fail closed. Do not use `PYTHONPATH`, bypass `-I`, or install untrusted bytes. Produce two clean source wheels with identical SHA; scoped-review intended contents/version; then update the authoritative wheel/download lock and canonical evidence to that exact SHA before atomic staging/install. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled parallel execution | Retain every active worker. Continue CONFIG, ROUTE, LABEL, Training, and Model Test integration in parallel with `UAT-TRAIN-INSTALL-001` setup/preflight/dry-run work. `VAL-TRAIN-BACKEND-001` is closed; do not repeat its evidence. `VAL-MODEL-005` remains static-only on correction hold; all unregistered executable paths remain blocked. VIEWER is accepted. |
