...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue source integration and non-launching checks for CONFIG, ROUTE, LABEL, Training, and Model Test. Executable use is limited to the two paths registered in `3ccdb29`; all other launch paths remain blocked. The current-machine Training environment remains qualified; defer only external fresh-machine installer qualification. VIEWER is accepted. |
