...
| `UAT-ROUTE-002` | FUNCTIONAL artifact owner; DESIGN protected-form owner by exact work order | Software validation PASS on `1f21a03`; changed-timing HIL remains | Decision/pulse occurs only at track finalization. Registered service tests prove zero pre-disappearance pulse; equality Unresolved/no pulse; Waste/Stop/pause/fault no late pulse; resolved Hit exactly one callback. Existing geometry, independent state, and zero coordinate persistence remain. |
...
| `VAL-TEST-HARNESS-ROUTE` | FUNCTIONAL Route owner; VALIDATION acceptance owner | Execution PASS 2/2 on integration `1f21a03` | Registered `sequence_test_service_test` and `live_sorting_service_test` passed with clean zero-window/dialog/crash/residual audit. No other path is authorized by this result. |
...
| `VAL-TRAIN-BACKEND-001` | FUNCTIONAL probe owner; VALIDATION registration/acceptance owner | Preparation authorized; execution prohibited pending separate static approval | Prepare exactly one non-UI QCore/service real-backend probe through the central harness. It must use exact OpenDSS Python, a tiny immutable fixture and disposable output, report truthful GPU effective device, prove progress plus bounded Stop or completion, preserve registry/source hashes, isolate a poisoned environment, and use no network. Configure/build and return the exact diff only; do not execute until separately registered and approved. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue source integration and non-launching checks for CONFIG, ROUTE, LABEL, Training, and Model Test. The six-path harness allowlist is unchanged; Training and Route registered pairs passed. `VAL-TRAIN-BACKEND-001` is not allowlisted. All other paths remain blocked. The current-machine Training environment remains qualified; defer only external fresh-machine installer qualification. VIEWER is accepted. |
