...
| `UAT-CONFIG-001` | FUNCTIONAL artifact owner; DESIGN protected-form owner by exact work order | Authority approved; initial-state blocker resolved; implementation begins after master/lock publication | Visible shell/panel/action name is `Configuration`. Add the sole approved Detector Configuration control, `Small-droplet rejection`, whose Set rectangle maps to source pixels and applies its area to the existing minimum contour-area threshold. The initial authoritative value is exactly `100 px²`; do not expose or retain `Automatic` or `-1` as the product default. Setup Profile stores only the current numeric threshold. It remains editable and applies immediately during active Runs, without Run/event/log/Results provenance. Preserve protected detector mechanics and use the smallest direct authoritative-state adapter. |
| `UAT-ROUTE-002` | FUNCTIONAL artifact owner; DESIGN protected-form owner by exact work order | Authority approved; tracking-timing correction begins after master/lock publication | `Decision Boundary` is operational. Explicit Set arms one source-image X/Y click; ordinary clicks are inert; the horizontal segment runs from that point to the RIGHT EDGE only. Top/Bottom selects Hit side, Reset clears, and Start is blocked while unset. All in-frame positions are tracking-only. Classification occurs only when the track ends/disappears, using final Y against boundary Y. Clicked X positions only the visible segment and is not an operational threshold. A track ending before X is still classified from final Y; no Unresolved outcome is introduced. Independent state and zero coordinate persistence remain. |
