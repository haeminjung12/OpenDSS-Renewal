...
This slice remains open only for user-led overall visual validation: Qt Design Studio 2D view and Live Preview at 1600 × 900, maximized, and a representative larger 16:9 size, including keyboard focus and non-color cues. GUI automation was not run. Main retains unrelated user-owned dirty `AGENTS.md` and orchestrator-skill edits, plus the protected untracked `Desktop_app_v2.qmlproject.qtds`; do not treat the tracked main worktree as clean.
