...
| `VAL-MODEL-005` | FUNCTIONAL Model Test correction owner; VALIDATION scoped-rereview/acceptance owner | Execution HOLD before tests; four registrations remain static-only | Correct only the confirmed blockers: forward OOM halves the batch and retries; a no-newline child cannot hang Stop/join; CSV plus partial-summary persistence has recoverable pair semantics. No registered target may execute until changed-source scoped rereview accepts the fixes. Do not broaden to product, QML, GUI, or real-Dataset execution. |
...
| `FINAL-UAT-001` | VALIDATION consolidated GUI/HIL owner; FUNCTIONAL frozen-candidate owner | Authorized after final candidate freeze | Perform one consolidated GUI/HIL pass only after the candidate is frozen. Announce exact Computer Use actions first. All hardware is connected; use Chrome oscilloscope `http://169.254.14.81/` in ROLL mode when physical trigger evidence is required. Reuse unchanged evidence and add no requirements. Integration and Design must not visibly launch. |
