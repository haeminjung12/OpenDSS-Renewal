...
| `UAT-ROUTE-HIL-001` | FUNCTIONAL Route correction owner; VALIDATION HIL acceptance owner | FAIL on `4d4da434`; release blocker; candidate quarantined | Physical Sequence Test run `...\runs\Sequence Test - 20260226_173838-8` produced rows with `observed_route=Waste` while every shown row recorded `daq_pulse_status=issued`; RIGOL CH1 showed matching physical pulses. CSV SHA `CDADB4CA4FB1B83E2C7CA024F322B90F7F89AEAD686A8749399A32457835315A`. This contradicts the required Waste=no-pulse arbitration and rejects candidate `4d4da434` for release. Stop completed, CH1 became flat/current 0.00 V, the app closed, and exact process count was zero. FUNCTIONAL owns exact root-cause diagnosis and the smallest arbitration correction, followed by changed-seam software tests and scoped review. Hold equality/pause HIL and perform no further HIL against the known-bad candidate. Preserve previously passed Hit timing, shutdown-zero, visible UAT, and unrelated evidence. |
| `FINAL-UAT-001` | VALIDATION consolidated GUI/HIL owner; FUNCTIONAL frozen-candidate/fix owner | Visible-control/layout pass preserved on `4d4da434`; release blocked by `UAT-ROUTE-HIL-001` | Frozen executable SHA `6F2CB565AEAC2DBDC69F15FA94DA486CCDC2D13AF5792FC16DF97A5966095D64` completed consolidated visible UAT with no remaining reproduced visible-control or layout blocker. Live moving-freshness is environment-inconclusive because the physical scene was black; nonblank retention and no-flicker passed. Preserve this unchanged evidence, but do not accept the candidate for release because the later Route HIL found physical Waste pulses. |
| `SCHED-001` | FUNCTIONAL coordination owner | Route release-blocker correction is the critical path | Preserve all closed Training, Model Test, Configuration, Viewer, window, and visible-UAT evidence. Diagnose and correct only the `UAT-ROUTE-HIL-001` Waste-pulse arbitration seam, run targeted changed-seam software tests and one scoped review, then build a corrected candidate for the remaining Route HIL cases. Do not stamp the installer, repeat unrelated gates, or broaden the correction. |
