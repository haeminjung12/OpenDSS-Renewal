...
| `MODEL-WHEEL-LOCK-001` | Documentation authority owns accepted artifact identity; FUNCTIONAL preserves evidence; VALIDATION controls any future execution | `MODEL-WHEEL-UPDATE-001` CLOSED as safe rollback, not success; no retry authorized | Controlling master: `ODSS-DES-002` §2.1.1—verification, provisioning, and environment-check failure must fail atomically and a partial environment is never usable. The exactly-once updater execution failed and stopped; accepted wheel identity remains SHA-256 `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796`, 63,123 B, 19 members. Exact pre-state equals post-state: package tree `1119ed2682076bd86e3e1f1adca0be1043c89fba232d46569334c299793a4b68`; dist-info tree `dfd20f47910072db9420536e2bc594e9a41f1ac3eda5410f7430334270a76ebe`; entry executable `47238e0eb6b7942bce13a997eec45276170f65f0a6977ae52288079b24170e7a`. The 37/37 environment closure is unchanged and no snapshot, updater, pip, or Python residual remains. The original thrown text was lost to app-output truncation; accepted-wheel production files are absent/mismatched, proving success validation was not reached and the new wheel is uninstalled. Preserve evidence; no retry, updater/Python/install execution, installed CLI/backend gate, or representative probe execution is authorized. Only read-only/static root-cause diagnosis may continue. Any correction must add persistent diagnostics outside every rollback root; any new live attempt requires separate corrected static review and explicit Validation authorization. |
| `VAL-MODEL-005-REP` | FUNCTIONAL exact probe writer; VALIDATION static/execution acceptance owner | Probe `fdbce62` static HOLD on four changed seams; execution dependency blocked | Controlling master: `ODSS-DES-002` §§13.3, 13.5–13.7—Model Test uses a compatible labeled Dataset through Python/PyTorch with GPU-first/CPU fallback, ordered per-image facts, completed-batch checkpoints, v3 checkpoint-plus-metadata provenance, and no ONNX attribution. Exact write set remains existing `model_test_production_model_probe.cpp` plus exact CMake registration; no new probe/helper/fixture/production file. Candidate `fdbce62` must correct exactly four changed seams: read the nested `artifact.checkpoint_sha256` declaration; prevent the probe from triggering any production-package metadata migration/mutation; treat disposable TEMP cleanup failure as fatal; and assert the actual/effective device rather than the planned device. Do not reopen other accepted fixture, ordering, provenance, hash-preservation, output-coherence, or containment behavior. No Python, install, or probe execution before `MODEL-WHEEL-LOCK-001` static PASS and this exact changed-lines correction receives scoped static PASS. |
