...
| `UAT-CONFIG-001` | FUNCTIONAL artifact/diagnostic owner; DESIGN protected-form owner; VALIDATION acceptance owner | C++ PASS/CLOSED; exactly one diagnostic Shell rerun authorized at `4ca4eeb` | Detector characterization passed in 0.27 s and `live_sorting_controller_test` in 1.62 s; do not repeat. `ShellSingleImage` failed with empty output. Diagnostic-only `4ca4eeb` is statically accepted solely to retain the results file on one registered Shell rerun; require zero relevant residual processes, windows, dialogs, or crashes. No other execution. Product behavior remains unchanged: visible `Configuration`; sole `Small-droplet rejection`; nearest-pixel mapping; exact `100 px²` initial/legacy value; profile-only current persistence; immediate active-Run effect; zero Run/event/log/Results provenance. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled parallel execution | Retain every active worker. Continue CONFIG, ROUTE, LABEL, Training, and Model Test integration in parallel with `UAT-TRAIN-INSTALL-001` setup/preflight/dry-run work. Training backend, Model Test C++, and Configuration C++ gates are closed; do not repeat their evidence. Continue `MODEL-WHEEL-LOCK-001`. Configuration may perform exactly one diagnostic registered Shell rerun at `4ca4eeb`; all other unregistered paths remain blocked. VIEWER is accepted. |
