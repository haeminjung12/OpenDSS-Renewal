...
The final approved structure now records Label's ordered 390 px outer-collapsible panel, switchable two-or-three-class setup for configured Datasets, Dataset Save As copy-and-load semantics, shared outer-panel behavior, Train and Model Test Results placement, and Settings Visuals/Text Size policy. Class 0/1/2 remain visible and Class 2 is disabled in two-class mode. These are documentation authority for the next visual/mock work; they are not implemented production behavior.
...
- Final-structure mocks use the shared 390 px expanded outer-panel width and whole-panel collapse; Label uses only Load Dataset, Dataset Summary, Label, Filter, and bottom-right Save As, keeps two/three classes switchable for configured Datasets, and shows Class 0/1/2 with Class 2 disabled in two-class mode; Train and Model Test keep Dataset Summary above a separate Results region; Settings contains only Storage, Application Information, Diagnostics, and Visuals with Text Size at 80%–200%, default 100%.
