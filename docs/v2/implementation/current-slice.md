...
| `VAL-TEST-HARNESS-ROUTE` | FUNCTIONAL Route owner; VALIDATION acceptance owner | Static registration PASS at `5b1735f`; execution evidence pending | Controlled execution is authorized only for registered `sequence_test_service_test` and `live_sorting_service_test`, using the shared offscreen helper and sanitized PATH. Each run must return zero-window/dialog/crash/residual-process evidence. This registration does not authorize another path. |
