...
| `MODEL-WHEEL-LOCK-001` | Documentation authority publishes accepted artifact identity; FUNCTIONAL reconciles exact lock/install path; VALIDATION accepts installed gates | Accepted artifact identity published; exact lock-field/updater writes released; execution still blocked | Correction `76a6b0b` (source `8f88acd`) received scoped FINAL REVIEW ACCEPT. The relocated source suite passed 66/66 and isolated/offline production CLI passed. Two clean wheels are byte-identical: SHA-256 `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796`, size 63,123 B, 19-member inventory digest `204da652fa9728b43a2a9ca885a55866cedb4ff3186484aaf3c3bcf4ccb52f93`, with zero tests, experiments, caches, docs, or data; evidence `D:\b\odmodelwheel-v2\MODEL-WHEEL-LOCK-001-evidence.txt`. Reject prior `b28f56b9…b85943` and `0783db6b…cf3d`. FUNCTIONAL may replace only the `droplet-trainer==0.2.0` hash in `training/python/requirements/windows-py312-gpu-cu130.lock` with the accepted full SHA. The download catalog's existing embedded-wheel filename and inventory's existing `0.2.0` version remain unchanged; do not invent duplicate hash fields. To avoid a redundant 37-distribution reprovision, FUNCTIONAL may add only `training/python/scripts/windows/update-training-trainer-wheel.ps1`: verify the published lock; snapshot the exact installed `droplet_trainer`, its dist-info, and entry scripts; install only the pinned local wheel with `--no-index --no-deps`; run the exact 37-distribution inventory plus isolated environment/CLI validation; and restore the snapshot on any failure. Do not create a generic updater or execute it before scoped static acceptance. After lock/updater acceptance, release exactly one atomic update and the installed production CLI/backend and representative-Dataset gates. |
