...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue CONFIG, ROUTE, LABEL, Training, and Model Test integration. `VAL-TRAIN-BACKEND-001` is closed; do not repeat its environment, controller, protocol, or real-backend evidence. All unregistered executable paths remain blocked. Defer only external fresh-machine installer qualification. VIEWER is accepted. |
