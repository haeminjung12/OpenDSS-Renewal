...
| `VAL-TRAIN-BACKEND-001` | FUNCTIONAL probe owner; VALIDATION registration/acceptance owner | Corrected static PASS at `3768e22`; exactly one registered execution authorized | Duplicate env-check removed; real `TrainingService` only; 105 s internal maximum under CTest timeout 150 s; cleanup occurs before every post-start return. Execute once through the registered central harness and prove the real child path, progress, and terminal state; unchanged fixture, registry, and weight hashes; and zero relevant residual processes, windows, dialogs, or crashes. Do not repeat green environment/controller tests. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue source integration and non-launching checks for CONFIG, ROUTE, LABEL, Training, and Model Test. Training and Route registered pairs passed; corrected `VAL-TRAIN-BACKEND-001` at `3768e22` is authorized for exactly one registered execution. All other unregistered paths remain blocked. The current-machine Training environment remains qualified; defer only external fresh-machine installer qualification. VIEWER is accepted. |
