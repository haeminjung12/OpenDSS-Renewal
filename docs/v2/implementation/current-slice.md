...
| `MODEL-WHEEL-LOCK-001` | FUNCTIONAL packaging-correction/deterministic-build owner; VALIDATION scoped-review owner | Exact production-only correction authorized; prior wheel rejected; lock/install blocked | Current two clean builds match at SHA `b28f56b9…b85943`, size 134142, 46 members, inventory digest `be55849f…b2183`; evidence `C:\Users\goals\AppData\Local\Temp\model-wheel-lock-001-evidence.json` is diagnostic-only. Reject and never publish that hash. Exact packaging writes: `training/python/pyproject.toml` and `training/python/droplet_trainer/__init__.py` only. Single-source exact internal/wheel version `0.2.0`; use an explicit production package/module whitelist; exclude `publication_experiments/**`, every test, caches, docs, and data. Installed-Python validation exercises the production module/CLI under `-I` plus offline denial and must not require shipped unittest modules. No trainer implementation edit, lock update, install, real Dataset, `PYTHONPATH`, or `-I` bypass. After scoped rereview, require two new clean identical-SHA builds and member review before authoritative wheel/download-lock publication and atomic staging/install. |
