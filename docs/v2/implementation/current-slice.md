...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue source integration and non-launching checks for CONFIG, ROUTE, LABEL, Training, and Model Test. Executable use is limited to smoke, `sequence_test_controller_test`, `sequence_test_service_test`, and `live_sorting_service_test` through harness `3ccdb29`; the two service tests remain execution-pending. All other paths remain blocked. The current-machine Training environment remains qualified; defer only external fresh-machine installer qualification. VIEWER is accepted. |
