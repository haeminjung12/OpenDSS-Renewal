...
| `MODEL-WHEEL-LOCK-001` | Documentation authority owns accepted artifact identity; FUNCTIONAL corrects exact diagnostic seam; VALIDATION controls review/execution | `MODEL-WHEEL-UPDATE-001` CLOSED safe; `MODEL-WHEEL-UPDATE-002-DIAG` exact static correction authorized; no live attempt | Controlling master: `ODSS-DES-002` §2.1.1—verification, provisioning, and environment-check failure must fail atomically and a partial environment is never usable. The first update remains CLOSED as safe rollback with the exact hashes recorded in commit `63a632e`; accepted wheel SHA-256 remains `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796`. Root cause is exact: installed `RECORD` stores `../../Scripts/droplet-trainer.exe`, while updater preflight required three `../`; `Get-InstalledTrainerLayout` therefore failed before snapshot/install, fully explaining pre-state=post-state and zero snapshots. `MODEL-WHEEL-UPDATE-002-DIAG` authorizes an isolated edit to existing `training/python/scripts/windows/update-training-trainer-wheel.ps1` only: correct that exact RECORD-depth match and persist phase, exception, and rollback diagnostics outside every rollback root. Do not alter accepted hash, transaction semantics, inventory checks, rollback scope, or other passed seams. No updater, pip, Python, install, installed CLI/backend, or representative-probe execution. A future live attempt requires changed-lines static FINAL ACCEPT and separate explicit Validation authorization. |
