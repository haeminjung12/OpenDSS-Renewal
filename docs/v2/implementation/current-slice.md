...
| `VAL-TRAIN-BACKEND-001` | FUNCTIONAL probe owner; VALIDATION registration/acceptance owner | Static REJECT at `04e51ba`; correction preparation only; execution prohibited | Rejected probe unnecessarily repeated the closed env-check and allowed internal waits of 120+180+15 s under CTest timeout 240 s, risking orphan Python. Correct minimally: reuse accepted env evidence; exercise real `TrainingService` only; retain exact OpenDSS Python, tiny immutable fixture/disposable output, truthful GPU device, progress plus bounded Stop or completion, registry/source-hash preservation, poisoned-env isolation, and no network; enforce a total bound with safe margin below CTest timeout and guaranteed post-start cleanup. Configure/build and return diff only; do not execute until separately approved. |
