...
| `UAT-ROUTE-HIL-001` | VALIDATION HIL owner; FUNCTIONAL final-candidate owner | PARTIAL; remaining physical zero-case matrix released | On final candidate `4d4da434`, physical evidence proves multi-frame Hit output stayed 0 before disappearance/finalization and then emitted one configured pulse. Final Stop/Clear, explicit scalar 0.0 V, and a flat scope afterward also passed. Focused software tests cover equality, Waste, Stop, and pause as zero-output cases, but these four cases were not each physically observed on the oscilloscope; therefore full HIL is not yet PASS. Execute only the missing physical equality/Waste/Stop/pause zero-output matrix on the preserved candidate, then finish with Stop/Clear/scalar-zero confirmation. Do not repeat Hit timing, GUI UAT, or unchanged waveform evidence. |
| `FINAL-UAT-001` | VALIDATION consolidated GUI/HIL owner; FUNCTIONAL frozen-candidate/fix owner | FINAL PASS/CLOSED on pushed `renewal/main` `4d4da434` | Frozen executable SHA `6F2CB565AEAC2DBDC69F15FA94DA486CCDC2D13AF5792FC16DF97A5966095D64` completed the consolidated visible UAT with no remaining reproduced acceptance blocker. Live moving-freshness is environment-inconclusive because the physical scene was black; nonblank retention and no-flicker behavior passed, so do not report a product defect or repeat without a materially observable scene. Process teardown was clean. Preserve all unchanged accepted evidence. |
