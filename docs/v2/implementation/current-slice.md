...
| `VAL-MODEL-005-REP` | FUNCTIONAL preserves and executes accepted probe; VALIDATION execution acceptance owner | Exactly one representative execution released at frozen `c4f1e47` | Controlling master: `ODSS-DES-002` §§13.3, 13.5–13.7. The accepted six-record probe closes the prior nested checkpoint, production-mutation, TEMP-cleanup, and effective-device seams. It additionally sorts and rejects any `*.partial*` artifact or `model_test_checkpoint.json` residue anywhere under `outputRoot` before cleanup. The accepted wheel installation and installed production CLI/backend gates have passed. Validation authorizes exactly one execution of the frozen `c4f1e47` probe. Do not change the probe or registration, rerun the updater/CLI gate, expand to the full Dataset, launch GUI, or permit network access. Return the exact six-record facts, effective-device truth, artifact/provenance coherence, source/registry/model hash preservation, residue audit, and terminal result. |
