...
| `VAL-MODEL-005-REP` | FUNCTIONAL corrected-probe execution owner; VALIDATION execution acceptance owner | Static PASS at `9965f44`; exactly one corrected representative execution released | Controlling master: `ODSS-DES-002` §§13.3, 13.5–13.7. Integrated `9965f44` changes exactly one line to fixture-local `sourceFrameIndex = index + 1` while preserving original `manifestIndex` in `sourceFrameId`/order evidence. Release target build and Validation changed-lines static review pass. Validation releases exactly one corrected representative execution through the existing registered sanitized/offline non-GUI harness. Return exact six-record facts, effective-device truth, coherent artifacts/provenance, unchanged source/registry/model hashes, clean residue audit, and terminal result. Do not alter probe/production/CMake, rerun updater/CLI gates, expand to the full Dataset, launch GUI, or permit network access. |
