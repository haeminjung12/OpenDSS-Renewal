...
| `VAL-MODEL-005` | FUNCTIONAL Model Test owner; VALIDATION acceptance owner | FINAL PASS/CLOSED; release combined candidate assembly | Summary passed in 0.46 s, writer in 11.05 s, and registered service in 3 s at `0dd0bf3`. Accepted wheel installation, installed production CLI/backend, and the representative six-record CUDA Dataset gate are also PASS/CLOSED in their rows below. All Model Test gates are closed and must not repeat. Release final combined candidate assembly. Remaining external validation is `FINAL-UAT-001` plus changed Route timing `UAT-ROUTE-HIL-001`. |
| `MODEL-WHEEL-LOCK-001` | Documentation authority owns accepted artifact identity; FUNCTIONAL preserves installed runtime; VALIDATION execution acceptance owner | Installed post-state and production CLI/backend PASS/CLOSED | Controlling master: `ODSS-DES-002` §2.1.1. The exact accepted wheel SHA-256 `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796` installed successfully at accepted updater commit `824c549`, exit 0. Installed bytes match all 14 production modules, including `model_test_batch_backend`; the 37-distribution closure passed; and no snapshot, temporary diagnostic, updater, pip, or Python residual remains. Installed evidence: METADATA `a4994de4…92f3`, RECORD `30e1abaf…5ff6`, entry point `6f8ecbfa…8935`, and authoritative command-log SHA `b988cd3a…77086`. The exactly-once sanitized/offline production gate passed using exact OpenDSS Python `-I -m droplet_trainer model-test-process --help`, exit 0; it imported the production batch backend and exposed the checkpoint surface without Dataset, model, GUI, or network access. Python SHA `0b471133…fc14`, output SHA `d5fd511c…d5a2`, zero Python/pip residue. Do not rerun the updater or this gate. The best-effort final JSON replacement left the prior failure JSON stale; that JSON is not current-state evidence and is governed by `MODEL-WHEEL-SUCCESS-LOG-001`. |
...
| `UAT-ROUTE-HIL-001` | FUNCTIONAL corrected-candidate owner; VALIDATION HIL acceptance owner | FINAL PASS/CLOSED on `7ac11368` / executable SHA `E32B15A3…97DF8` | Corrected physical Waste PASS: 454 processed frames and 15/15 events recorded `observed_route=Waste` with `daq_pulse_status=not_requested`; RIGOL remained flat at 0.00 V. CSV `…\Sequence Test - 20260226_173838-10\events.csv`, SHA `76D47C42C3F3B3432DAE3EFD58BCEBE376E3921D5E52E8D39E9BF90769CCA0D4`. Separate physical Stop PASS at 1 FPS, flat 0.00 V. Physical Live pause PASS after 7 s with zero droplets, flat 0.00 V; Stop artifact status was stopped/user with DAQ enabled and zero events, CSV SHA `6C9934…F3A1E`, summary SHA `922585…C1BE9`. Final camera Stop, DAQ disable, scalar zero, app close, and exact zero process count passed. Exact equality cannot be physically forced without guessing or prohibited exposure/persistence of final tracked Y; accept the deterministic registered software proof that equality yields Unresolved/Suppressed/no callback, corroborated by the physical adjacent zero-output cases. No instrumentation or repeat. |
| `FINAL-UAT-001` | VALIDATION consolidated GUI/HIL owner; FUNCTIONAL frozen-candidate/fix owner | FINAL PASS/CLOSED; unchanged evidence reused on corrected candidate | Visible UAT/window evidence from executable SHA `6F2CB565…95D64` remains accepted because correction `7ac11368` is confined to Sequence finalized-route DAQ arbitration and persistence validation. Corrected executable SHA `E32B15A3…97DF8` passes its changed Route HIL seam. Live moving-freshness remains environment-inconclusive from the black physical scene; nonblank retention/no-flicker passed. No remaining reproduced acceptance blocker; do not repeat. |
| `SCHED-001` | User acceptance owner; FUNCTIONAL installer owner | Program acceptance gates closed; awaiting user acceptance before final installer stamp | Corrected candidate `7ac11368`, executable SHA `E32B15A3…97DF8`, has no remaining reproduced acceptance blocker. Preserve all evidence and do not repeat gates. The final stamped installer remains blocked until the user explicitly accepts this almost-final executable; fresh-machine qualification remains deferred. |
