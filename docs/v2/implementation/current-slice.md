...
| `VAL-TEST-HARNESS-TRAIN` | FUNCTIONAL Training owner; VALIDATION acceptance owner | Static registration PASS at `ac7c9f1`; execution evidence pending | Controlled execution is authorized only for `training_service_protocol_test` and `training_controller_test` through the shared harness. Their self-hosted QCore fake-trainer child modes are included. Each run must audit zero parent/child residual processes, windows, dialogs, and crashes. No real Python, backend, or product execution is authorized. |
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled harness-only execution | Retain every active worker. Continue source integration and non-launching checks for CONFIG, ROUTE, LABEL, Training, and Model Test. The harness allowlist is smoke, `sequence_test_controller_test`, `sequence_test_service_test`, `live_sorting_service_test`, `training_service_protocol_test`, and `training_controller_test`; Route and Training registrations remain execution-evidence pending. All other paths remain blocked. The current-machine Training environment remains qualified; defer only external fresh-machine installer qualification. VIEWER is accepted. |
