...
| `UAT-CONFIG-BLK-001` | User decision | Blocked | Channel Reference Line presentation is defined beneath Decision Boundary (`Set`, click vertical frame position, horizontal line, `Reset`), but whether it is display-only or affects detector/routing decisions is unresolved. No implementation, persistence, provenance, detector/routing input, or validation expectation may infer either interpretation. |
...
3. `UAT-CONFIG-BLK-001` remains excluded until the user decides whether Channel Reference Line is display-only or operational.
4. No worker may add width/height rejection, a second detector state, Run provenance for in-run threshold changes, boundary coordinate persistence/history, detector-mechanics replacement, speculative Channel Reference Line behavior, thumbnail zoom, or duplicated hot-path viewer JavaScript.
