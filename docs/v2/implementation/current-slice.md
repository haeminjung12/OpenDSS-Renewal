...
| `MODEL-WHEEL-LOCK-001` | Documentation authority publishes accepted artifact identity; FUNCTIONAL executes exact accepted updater; VALIDATION execution acceptance owner | Updater scoped FINAL ACCEPT at `1d13ae3`; `MODEL-WHEEL-UPDATE-001` authorized exactly once | Controlling master: `ODSS-DES-002` §2.1.1—verification, provisioning, and environment-check failure must fail atomically and a partial environment is never usable. Accepted wheel SHA-256 remains `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796`; updater `1d13ae3` closes all three blockers from `f2c5e24` and has scoped FINAL ACCEPT. Validation authorizes exactly one atomic update of `%LOCALAPPDATA%\OpenDSS\training-venv-gpu` using the accepted updater chain and exact published run1 wheel SHA. Require exact successful post-state or exact rollback proof, plus zero relevant residual processes; on failure stop and do not retry automatically. Installed CLI/backend evidence and `VAL-MODEL-005-REP` execution remain separately held pending post-state acceptance and their own explicit releases. Do not rerun prior source/C++/environment gates. |
