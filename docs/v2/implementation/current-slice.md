...
| `MODEL-WHEEL-LOCK-001` | Documentation authority owns accepted artifact identity; FUNCTIONAL corrects exact parser seam; VALIDATION controls review/execution | `MODEL-WHEEL-UPDATE-002` FAILED SAFE; exact rollback accepted; `UPDATE-003-DIAG` static correction only | Controlling master: `ODSS-DES-002` §2.1.1. The exactly-once second attempt failed inventory validation (`expected=37`, `actual=1`) after snapshot/install; validation did not pass; rollback restored and verified exact prior package/dist-info/entry hashes, 37/37 closure, cleanup, and zero snapshot/updater/pip/Python residuals. No retry is authorized. Root cause is exact Windows PowerShell 5.1 behavior: `@($actualJson | ConvertFrom-Json)` retains the parsed JSON array as one pipeline item, while assigned-first parsing enumerates all 37 distributions. `MODEL-WHEEL-UPDATE-003-DIAG` authorizes only that exact assigned-first parsing correction in existing `update-training-trainer-wheel.ps1`; preserve durable diagnostics and every accepted transaction, hash, inventory, rollback, and cleanup seam. No updater/live/Python/install/CLI/backend/representative execution. A future attempt requires changed-lines static FINAL ACCEPT and separate explicit Validation authorization. Accepted wheel SHA remains `20e070bf0e5a114bf9daadaec22ff81a704c754d8077a7cea7af93d9dddab796`. |
