...
| `UAT-CONFIG-001` | FUNCTIONAL artifact owner; DESIGN protected-form owner; VALIDATION acceptance owner | Static/scoped review PASS after `02ecdc8`; three registered checks authorized | Integrated candidate `17bb4b0`; `02ecdc8` sanitizes the exact Live controller `PATH`; scoped implementation review FINAL ACCEPT. Execute only detector characterization, `live_sorting_controller_test`, and `ShellSingleImage` through registered harnesses; require zero relevant residual processes, windows, dialogs, or crashes. No other path is reopened. Product behavior remains unchanged: visible `Configuration`; sole `Small-droplet rejection`; nearest-pixel mapping; exact `100 px²` initial/legacy value; profile-only current persistence; immediate active-Run effect; zero Run/event/log/Results provenance. |
...
| `SCHED-001` | FUNCTIONAL coordination owner | Active delivery order; controlled parallel execution | Retain every active worker. Continue CONFIG, ROUTE, LABEL, Training, and Model Test integration in parallel with `UAT-TRAIN-INSTALL-001` setup/preflight/dry-run work. Training backend and Model Test C++ gates are closed; do not repeat their evidence. Continue `MODEL-WHEEL-LOCK-001`. Configuration may execute only its three accepted registered checks; all other unregistered executable paths remain blocked. VIEWER is accepted. |
