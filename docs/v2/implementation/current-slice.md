...
| `UAT-MODEL-005` | FUNCTIONAL artifact owner | Authority approved; highest-priority parallel backend lane after master/lock publication | Existing Model Test/Dataset Validation semantics, artifacts, hashes, atomic partial persistence, and recovery remain unchanged. Automatically select the largest qualified memory-safe batch; expose no user batch setting. Preserve per-image order/facts; count progress only after an atomic completed-batch checkpoint; Stop starts no later batch; reduce batch size on pre-inference allocation failure. Proceed whenever an independent worker slot is available and do not leave this lane paused behind unrelated UI work. |
